...
public class ProtoSchemaManager {
    ...
    public void onSchemaDescriptorRegistered(@Observes SchemaDescriptorRegisteredEvent event) {
        SchemaRegisteredEvent schemaRegisteredEvent = event.getDescriptor().map(desc -> {
            ...
            try {
                descriptor = event.getMessageDescriptorGetter().apply(fullTypeName);
            ...
            return new SchemaRegisteredEvent(new SchemaDescriptor(processId + ".proto", event.getRawSchema(), new ProcessDescriptor(processId, fullTypeName)), SCHEMA_TYPE);
        }).orElseGet(() -> new SchemaRegisteredEvent(new SchemaDescriptor(event.getFileName(), event.getRawSchema(), null), SCHEMA_TYPE));
